...
- **太阳节气链路**：有外部参考源，当前口径是“对照 JPL Horizons，在 fixture 覆盖年份内容忍度 ±15 分钟”。
- **朔望链路**：当前主要是 bootstrap 回归样例，口径是“当前实现稳定 + 目标黄经差字段正确 + 事件时刻容忍度 ±6 小时”，不是独立外部精度报告。
- **中国历法链路**：当前主要是 modern 规则集下的离散边界样例，口径是“当前规则在这些样例上稳定可复核”，不是历史历书全量对齐声明。
