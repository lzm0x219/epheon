...
| D2.4 | 进行中 | leap second provider 非法值已覆盖，Delta-T 与 time-scale 窄测试待补。   |
...
说明：`parseUTCDateTime` 支持 `Z` 与 `±HH:mm`。还需要测试极端合法值与非法格式。
...
说明：当前测试覆盖 provider 返回 `NaN` 与抛错。还应覆盖正负 Infinity。
