...
| C4b  | M    | 已完成 | C1、C2、C4   | 月亮黄经最小 provider 与 bootstrap lunar fixture            |
| C5   | M    | 已完成 | B3、C3、C4   | 二十四节气求解                                              |
| C6   | L    | 已完成 | B3、C3、C4b  | 朔望求解                                                    |
| C7   | XL   | 进行中 | C5、C6       | 中国历法规则包                                              |
...
截至 `2026-06-18`，第二阶段新增进展：

```txt
C7：@epheon/calendar-chinese 包骨架
C7：月序构建与闰月判定最小切片
```
...
```txt
packages/ephemerides-elp2000/
standards/lunar/
```

说明：实现朔望求解所需的最小月亮黄经能力。当前包名沿用架构中的 `ephemerides-elp2000` 目标位，但实现仍保持 bootstrap 级别，不在此任务中引入完整 ELP2000 系数展开。
...
```txt
当前公共入口导出 createELP2000MoonProvider()、lunarEclipticLongitude()、lunarEclipticPosition()。
provider 当前只支持 Body.Moon、ReferenceFrame.MeanOfDateEcliptic、ReferenceFrame.TrueOfDateEcliptic 与 Precision.Standard。
实现使用低阶月球黄经 / 纬度 / 距离近似模型，并叠加简化章动黄经修正，先提供朔望求解所需的稳定月亮黄经输入。
tests/moon-provider.test.ts 当前联用太阳 provider，并以 standards/lunar/phases.json 中的 bootstrap 朔 / 望样例验证日月黄经差。
```

验收标准：

- 输入 `JulianEphemerisDay` 或 `Instant`，输出 `Angle` 或 `Position(Body.Moon)`
- 实现 `EphemerisProvider.position(Body.Moon, instant, options)`
- 不支持的天体返回 `EphemerisError("UnsupportedBody")`
- 测试读取 `standards/lunar/phases.json`
- 误差目标写入测试 tolerance，不隐藏默认精度
...
```txt
当前公共入口新增导出 LunarPhaseKind、LunarPhaseEvent 与 findLunarPhaseBetween()。
实现固定支持 NewMoon 与 FullMoon，显式请求月亮 / 太阳地心视黄经（Body.Moon、Body.Sun + ReferenceFrame.TrueOfDateEcliptic），先按 6 小时步进寻找包围区间，再用二分法细化到秒级。
tests/lunar-phases.test.ts 当前使用 standards/lunar/phases.json 中的 bootstrap 朔 / 望样例验证时刻，并显式声明 6 小时时间 tolerance。
```

验收标准：

- 提供 `findLunarPhaseBetween(kind, start, end, context)`
- 返回结果包含朔望类型、目标日月黄经差与 `Instant`
- 使用注入的 ephemeris provider 与显式 context
- 测试从 `standards/lunar/phases.json` 验证

### C7：中国历法规则包

文件范围：`packages/calendar-chinese/`
...
```txt
packages/calendar-chinese/
```

当前实现说明：

```txt
当前先创建 @epheon/calendar-chinese 包骨架，并落下最小规则切片 buildLunarMonthSequence()。
该函数当前只做两件事：用连续朔时刻切出月段；把第一个不含中气的月段标记为闰月。
tests/lunar-months.test.ts 当前使用合成朔 / 中气时刻验证月序边界与闰月判定，不提前进入完整农历日期、干支或历史规则集。
```

下一步：

- 基于节气与朔望真实结果补月编号与月份归属规则
- 再进入完整农历日期与干支推导
